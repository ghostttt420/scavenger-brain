...
def run_dummy_generation():
    ...
    # --- CRITICAL FIX: CREATE MASK ---
    map_mask = pygame.mask.from_surface(phys_map)
    ...
    for i in range(300):
        ...
        if not alive: break
        camera.update(alive[0]) # Follow first alive
        
        for c in cars:
            if c.alive:
                if random.random() < 0.1: c.steering = random.choice([-1, 0, 1])
                c.input_gas()
                c.update(map_mask, skid_map) # Pass MASK, not surface
        
        screen.fill(simulation.COL_BG)
        try:
            screen.blit(vis_map, (0,0), area=camera.camera)
            screen.blit(skid_map, (0,0), area=camera.camera)
        except: pass
        for c in cars: c.draw(screen, camera)
        pygame.display.flip()
        try:
            pixels = np.transpose(pygame.surfarray.array3d(screen), (1, 0, 2))
...
def run_simulation(genomes, config):
    ...
    # --- UNPACK & MASK ---
    start_pos, phys_map, vis_map, skid_map, checkpoints, start_angle = map_gen.generate_track()
    map_mask = pygame.mask.from_surface(phys_map) # <--- THIS IS THE FIX
    ...
    if GENERATION == START_GEN + 1 or GENERATION % 10 == 0 or GENERATION >= FINAL_GEN:
        writer = imageio.get_writer(os.path.join(VIDEO_OUTPUT_DIR, f"gen_{GENERATION:05d}.mp4"), fps=FPS)
    ...
    while len(cars) > 0 and frame_count < max_frames:
        ...
        # Camera logic: Follow the BEST alive car
        ...
        for i, car in enumerate(cars):
            ...
            # Inputs
            car.check_radar(map_mask) # Pass MASK
            radars = [0]*5 if len(car.radars)<5 else [d[1]/simulation.SENSOR_LENGTH for d in car.radars]
            gps = car.get_data(checkpoints)
            
            output = nets[i].activate(radars + gps)
            if output[0] > 0.5: car.input_steer(right=True)
            elif output[0] < -0.5: car.input_steer(left=True)
            car.input_gas()
            
            # Update Physics (Pass MASK)
            car.update(map_mask, skid_surface=skid_map)

            # Fitness
            if car.check_gates(checkpoints): ge[i].fitness += 200
            ge[i].fitness += (1.0 - gps[1]) * 0.1 # Distance to center bonus
            ge[i].fitness += car.speed * 0.05     # Speed bonus
        ...
        for i in range(len(cars) - 1, -1, -1):
            if not cars[i].alive:
                cars.pop(i); nets.pop(i); ge.pop(i)
        ...
        if writer or frame_count % 10 == 0:
            ...
            screen.blit(font.render(f"GEN {GENERATION}", True, (255,255,255)), (20, 20))
            pygame.display.flip()
